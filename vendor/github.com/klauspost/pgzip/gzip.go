...
func NewWriter(w io.Writer) *Writer {
	z, _ := NewWriterLevel(w, DefaultCompression, Gzip)
	return z
}

func NewWriterZlib(w io.Writer) *Writer {
	z, _ := NewWriterLevel(w, DefaultCompression, Zlib)
	return z
}
...
func (z *Writer) init(w io.Writer, level int, format CompressionFormat) {
	z.wg.Wait()
	digest := z.digest
	if digest != nil {
		digest.Reset()
...
func beput2(p []byte, v uint16) {
	p[0] = uint8(v >> 8)
	p[1] = uint8(v)
}

func beput4(p []byte, v uint32) {
	p[0] = uint8(v >> 24)
	p[1] = uint8(v >> 16)
	p[2] = uint8(v >> 8)
	p[3] = uint8(v)
...
func (z *Writer) writeZlibHeader() (int, error) {
	var compLevel int
	if z.level >= BestCompression {
		compLevel = 3
	} else if z.level == BestSpeed {
		compLevel = 0
	} else if z.level >= 6 || z.level == -1 {
		compLevel = 1
	} else {
		compLevel = 2
	}
	...
	var header uint16
	header = uint16(0x78<<8) + uint16(compLevel<<6)
	header += uint16(31) - (header % 31)
	beput2(z.buf[0:2], uint16(header))
	n, err := z.w.Write(z.buf[0:2])
	if err != nil {
		z.pushError(err)
		return n, err
	}
...
func (z *Writer) writeGzipHeader() (int, error) {
	z.buf[0] = gzipID1
	z.buf[1] = gzipID2
	z.buf[2] = gzipDeflate
	z.buf[3] = 0
...
func (z *Writer) Write(p []byte) (int, error) {
	...
	if !z.wroteHeader {
		...
		if z.compType == Gzip {
			z.writeGzipHeader()
		} else if z.compType == Zlib {
			z.writeZlibHeader()
		} else {
			return 0, fmt.Errorf("invalid compression type")
		}
...
func (z *Writer) Close() error {
	...
	close(z.results)
	digest := z.digest.Sum32()
	if z.compType == Gzip {
		put4(z.buf[0:4], digest)
		...
		beput4(z.buf[0:4], digest)
		_, err := z.w.Write(z.buf[0:4])
		if err != nil {
			z.pushError(err)
			return err
		}
